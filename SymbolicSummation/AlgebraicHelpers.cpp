...
Expression simplifySubExpressions(ExpressionObject* target) {
    ...
    total = first;
    if(isMul)
        remainder = removeElementMultiplicatively(remainder.get(), first.get());
    Expression second = getElementOfType(remainder.get(), typeToGet);
    while(second.getTypeHash() != NULLTYPE) {
        if(isMul) {
            remainder = removeElementMultiplicatively(remainder.get(), second.get());
            total = distribute(total.get(), second.get());
        }
        
        second = getElementOfType(remainder.get(), ADDTYPE);
...
Expression simplify(ExpressionObject* target) {
    ...
    if(overallSign)
        result = -result;
    result = simplifySubExpressions(result.get());
    result.setSimplified(true);
